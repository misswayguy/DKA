...
##记得修改flm_calculation 25， 40行 需要添加visual prompt
...
if __name__ == '__main__':
    ...
    p.add_argument('--dataset', choices=["cifar10", "cifar100", "abide", "dtd", "flowers102", "ucf101", "food101", "gtsrb", "svhn", "eurosat", "oxfordpets", "stanfordcars", "sun397", "covid"], required=True)
    ...
    loaders, configs = prepare_expansive_data(args.dataset, data_path=data_path[args.dataset])
    ...
    vit_network = vit_b_16(weights=ViT_B_16_Weights.IMAGENET1K_V1)
    visual_prompt = ExpansiveVisualPrompt(224, mask=configs['mask'], normalize=normalize, network=vit_network).to(device)
